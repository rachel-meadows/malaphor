...
Takes these idioms and uses them to make a SQLite database with key info about each proverb.
...
idiomList = open("proverbList.txt", "r")
proverbs = json.load(open("proverbList.txt"))
...
for proverb in proverbs:
    ...
    # See if proverb is already in the database, and skip if it is.
    result = None
    cur.execute("select EXISTS(select 1 FROM Idiom WHERE idiom = ? )", (proverb , ))
    result = cur.fetchall()
    proverbInDatabase = result
    if proverbInDatabase[0] == (1,): # There is a record in the database - go to next proverb
        alreadyLogged += 1
        continue

    time.sleep(1) # Reduce load
    try:
        jsonRaw = parser.fetch(proverb.lower())
    except:
        continue
    ...
    cur.execute('''INSERT OR IGNORE INTO Idiom (idiom, global_pos)
        VALUES ( ?, ? )''', ( proverb, global_pos) )

    # Put values in the Related table

    # Put in synonyms
    for i in synonyms:
        cur.execute('SELECT id FROM Idiom WHERE idiom = ? ', (proverb , ))
        proverb_id = cur.fetchone()[0]
        cur.execute('''INSERT OR IGNORE INTO Related (idiom_id, related, type)
            VALUES ( ?, ?, ?)''', ( proverb_id, i, "synonym" ) )
    ...
    for i in derived_terms:
        ...
        proverb_id = cur.fetchone()[0]
        cur.execute('''INSERT OR IGNORE INTO Related (idiom_id, related, type)
            VALUES ( ?, ?, ?)''', ( proverb_id, i, "derived_terms" ) )
    ...
    for i in alternative_terms:
        ...
        proverb_id = cur.fetchone()[0]
        cur.execute('''INSERT OR IGNORE INTO Related (idiom_id, related, type)
            VALUES ( ?, ?, ?)''', ( proverb_id, i, "alternative_terms" ) )
    ...
    for i in related_terms:
        ...
        proverb_id = cur.fetchone()[0]
        cur.execute('''INSERT OR IGNORE INTO Related (idiom_id, related, type)
            VALUES ( ?, ?, ?)''', ( proverb_id, i, "related_terms" ) )
